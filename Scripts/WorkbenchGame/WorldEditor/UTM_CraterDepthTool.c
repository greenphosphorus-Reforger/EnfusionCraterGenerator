...
{
	[Attribute("22000", UIWidgets.Slider, "Total number of craters to scatter across the map.", "1000 200000 1", category: "Craters")]
	...
	[Attribute("0", UIWidgets.EditBox, "Surface mask pixel resolution. Check Terrain Tool > Info & Diags > Surface Map > Total and enter the first number (e.g. 4537)", category: "Craters")]
	protected int m_iMaskSize;
	...
	protected void WriteGeneratorScript()
	{
		...
		fh.WriteLine("import os as _os");
		fh.WriteLine("EXPORT_DIR = _os.path.join(_os.path.expanduser('~'), 'Documents', 'My Games', 'ArmaReforgerWorkbench', 'profile', 'export')");
		...
		fh.WriteLine("    p.add_argument(\"--hm-size\",       type=int,   default=2305)");
		fh.WriteLine("    p.add_argument(\"--map-metres\",    type=float, default=4608.0)");
		fh.WriteLine("    p.add_argument(\"--mask-size\",     type=int,   default=4537)");
		...
		fh.WriteLine("HM_SIZE         = _args.hm_size");
		fh.WriteLine("MAP_METRES      = _args.map_metres");
		fh.WriteLine("MASK_SIZE       = _args.mask_size");
		fh.WriteLine("");
		...
		fh.WriteLine("");
		fh.WriteLine("CELL_SIZE   = MAP_METRES / (HM_SIZE - 1)");
		fh.WriteLine("M_PER_PX    = MAP_METRES / MASK_SIZE");
		fh.WriteLine("");
		...
		fh.WriteLine("    fade_start = max(0.0, r_px - r_px * 0.15)");
		fh.WriteLine("    fade_end   = r_px + r_px * 0.35");
	...
	protected void GenerateCraters()
	{
		...
		// 3. Read terrain dimensions from active world
		int hmSize = m_API.GetTerrainResolutionX(0);
		float mapMetres = m_API.GetTerrainUnitScale(0) * (hmSize - 1);
		Print("[UTM Crater Tool] Detected: HM_SIZE=" + hmSize + "  MAP_METRES=" + mapMetres + "  MASK_SIZE=" + m_iMaskSize, LogLevel.NORMAL);

		if (m_iMaskSize <= 0)
		{
			Print("[UTM Crater Tool] ERROR: Mask Size is not set. Go to Terrain Tool > Info & Diags > Surface Map > Total and enter the first number into the Mask Size field.", LogLevel.ERROR);
			return;
		}
		...
		if (!pyCheck)
		{
			...
			Print("[UTM Crater Tool] Python not found. Downloading and installing automatically, please wait...", LogLevel.NORMAL);
			ProcessHandle installHandle = Workbench.RunProcess("powershell -Command \"Invoke-WebRequest -Uri 'https://www.python.org/ftp/python/3.11.9/python-3.11.9-amd64.exe' -OutFile '$env:TEMP\\python_installer.exe'; Start-Process '$env:TEMP\\python_installer.exe' -ArgumentList '/quiet InstallAllUsers=0 PrependPath=1 Include_pip=1' -Wait\"");
			if (installHandle)
			{
				Workbench.WaitProcess(installHandle, 300000);
				Workbench.FinishProcess(installHandle);
			}
			Print("[UTM Crater Tool] Python installed. Continuing...", LogLevel.NORMAL);

			// Verify Python now works
			ProcessHandle pyVerify = Workbench.RunProcess("python --version");
			if (!pyVerify)
			{
				Print("[UTM Crater Tool] ERROR: Python installation failed. Please install manually from https://python.org — check \"Add Python to PATH\" during install, then restart Workbench and try again.", LogLevel.ERROR);
				return;
			}
			Workbench.WaitProcess(pyVerify, 5000);
			Workbench.FinishProcess(pyVerify);
		}
		else
		{
		...
		cmd += " --hm-size "      + hmSize;
		cmd += " --map-metres "   + mapMetres;
		cmd += " --mask-size "    + m_iMaskSize;
